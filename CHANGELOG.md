...
- [test] `system.request` + `system.fetch` 后台心跳方案在 Watch S4 实机上**同样无效**
  - 现象：退到后台后，手表表盘上方显示应用 Logo（说明系统识别到后台声明），但定时器被冻结，到达间隔后不震动
  - 根因：S4 的 Vela 对第三方快应用的后台策略比官方文档描述更严格——即使声明了后台接口且接口"正在使用"，JS 执行线程仍被挂起
  - 官方文档列的三种后台接口（`system.audio` / `system.request` / `system.geolocation`）在 S4 上对第三方快应用均不生效
- [conclusion] **Watch S4 纯快应用形态无法实现后台定时提醒**，属平台级硬限制。已穷尽官方文档所有手段
- [note] 测试参数已恢复正式版（心跳 30s、间隔下限 15min）

**最终可用方案：**

| 方案 | 状态 | 说明 |
|------|------|------|
| 手表前台提醒 | ✅ 可用 | app 打开时按间隔震动，睡眠免打扰 |
| 手表后台提醒 | ❌ 不可行 | S4 平台硬限制，三条路全堵死 |
| 手机伴侣网页版 | ✅ 兜底 | `phone-companion/index.html` |
| 系统闹钟兜底 | ⚠️ 手动 | 手表自带闹钟/倒计时 |

**新 Logo：**

- [feat] 新增 [scripts/gen-logo.js](scripts/gen-logo.js)：零依赖 Node 脚本，生成 128×128 PNG
  - 深色圆形底 + 蓝青渐变水滴 + 高光反射 + 右上角铃铛提醒标记
  - 更贴合「督促饮水」的应用定位，表盘上方常驻显示时更直观
- [chore] [src/common/logo.png](src/common/logo.png) 更新为新设计
- [chore] 参数恢复：心跳 10s→30s，间隔下限 1min→15min
